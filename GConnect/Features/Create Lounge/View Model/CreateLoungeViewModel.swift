...
class CreateLoungeViewModel{
    ...
    func createLoungeData(game: String, title: String, role: [Bool], rank: String, gender: String, desc: String, uid: String, valid: @escaping (String) -> Void){
        ...
        if game == ""{
            ...
            db.collection("LoungeDetail").document().setData(["CreateAt":currentTime, "Desc":desc, "Game":game, "Title":title, "idMemberLounge":["Member1":uid, "Member2":"", "Member3":"", "Member4":"", "Member5":"", "Member6":"", "Member7":"", "Member8":"", "Member9":"", "Member10":""], "idRequirementsLounge":["Sentinel":role[0], "Initiator":role[1], "Controller":role[2], "Duelist":role[3], ], "Gender":gender, "Rank":rank]) {(error) in
                if error != nil{
                    print("Gagal")
                } else {
                    print("Sukses")
